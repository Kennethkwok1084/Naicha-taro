...
**版本**：V1.0.0 (MVP)
**负责人**：小菊 (PM/QA)
**周期**：2025.11.20 - 2025.11.30（10 天冲刺）
**目标**：完成前端开发与联调，具备上线能力
...
**核心目标**：10 天内完成“镇上知名奶茶店”小程序的 MVP 开发与联调，实现线上点单、自提/配送闭环。
...
* 交付准时：**11/30** 输出可供测试/上线的 Release 包。
...
| M1 | 11.21 | 基建完成       | Pinia/Request 封装、支付商户号提交（Owner: 小菊，DDL 11/25）、埋点框架 | 登录拿到 Token；lint 0 错；基础埋点可用    |
| M2 | 11.26 | 业务攻坚       | 菜单/购物车/结算页开发完毕                                     | 能加购；`preview` 调通；E2E 主链路通过    |
| M3 | 11.29 | 支付联调       | 微信支付联调畅通、订单状态流转                                    | 真机可唤起支付；状态更新正确；P0=0           |
| M4 | 11.30 | Dev Freeze | 提测包（Release Candidate）                             | Checklist 全绿；回滚演练通过（10 分钟内恢复） |
...
* 菜单页：左右联动、懒加载、加购入口
* 规格 Popup：默认选中规则（中杯/常规糖/常规冰）、库存置灰、热饮隐藏冰度
* 购物车：增减数量、失效项置灰、清空与重算
* 结算页（重头戏）：地址选择（地图/手动）、`/orders/preview` 防抖（≥200ms）、优惠券面板 + 集点展示
* 支付流程：`X-Idempotency-Key` 生成、`requestPayment`、成功/取消分支
...
gantt
    ...
    section 准备
    资质/商户号申请       :active, a1, 11-20, 3d
    项目初始化/基建       :active, a2, 11-20, 2d
    
    section 后端
    Preview算价接口      :active, b1, 11-20, 3d
    支付回调/配置        :b2, 11-23, 2d
    
    section 前端
    菜单与规格Popup      :active, c1, 11-21, 3d
    购物车逻辑           :c2, 11-23, 2d
    结算与Preview对接    :crit, c3, 11-24, 3d
    支付联调             :c4, 11-27, 2d
    
    section 交付
    真机测试/BugFix      :d1, 11-29, 2d
    Dev Freeze/发版      :milestone, 11-30, 0d
```
...
* **V1.0.0 (MVP)**：融合 10 天冲刺计划 + 监控/灰度/回滚/Checklist；强化里程碑 Exit Criteria；杯型 UI 隐藏但参与计价。
