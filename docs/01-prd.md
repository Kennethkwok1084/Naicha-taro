# 🧋 奶茶点单小程序 PRD（Taro 版 · v1.0.0-RC）

**Owner**：小菊
**目标平台**：微信小程序
**技术栈**：Taro 3 + React + TypeScript
**UI 组件库**：Taroify（必要时少量接入原生小程序组件）
...
* **规格弹层**（Taroify：`Popup` + `Radio/Checkbox` + `Stepper`）：
...
* **支付**：`Taro.requestPayment`；下单超时 15 分钟未付→自动取消（前端展示倒计时）。
* **支付后**：
...
1. 自提 Happy Path：加购→结算→无地址→支付成功→显示取餐码。
2. 外送 Happy Path：选点 9.9km→通过→支付成功→显示预计送达时间。
3. 超距：选点 10.1km→阻断→引导改为自提。
4. 售罄：某加料售罄→选项置灰，仍可下单其他规格。
5. 价格变更：`preview` 返回 `PRICE_CHANGED`→二次确认继续下单。
6. 优惠券失效：`COUPON_INVALID`→清除券重算成功。
7. 超时未付：15 分钟后订单自动取消，前端状态同步。
...
* v1.0.0-RC（2025-11-20）：迁移 **Taro + React + TS**；积分改为**集点（满10送1）**且仅展示；外卖默认开启；环境写死 `dev 本地/ prod guajunyan.top`；补齐错误码与 E2E 用例。
