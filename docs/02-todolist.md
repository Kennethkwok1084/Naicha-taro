...
| M1: 基建完成 | 11.21 | 🔄 进行中 | 登录拿到Token、lint通过、基础埋点可用 |
...
### 1. 基建与公共层 【11.20-11.22】

#### 1.1 网络层封装
- [ ] **创建 `src/utils/request.ts`**
  - [ ] 封装 `Taro.request` 拦截器
  - [ ] Token 自动注入 (从 Storage 读取)
  - [ ] `X-Idempotency-Key` 生成逻辑 (UUID v4)
  - [ ] 统一错误码处理与 Toast 提示
  - [ ] 超时配置 (默认 10s)
  - [ ] 环境切换逻辑 (dev/staging/prod)
  
- [ ] **创建 `src/config/env.ts`**
  - [ ] dev 环境配置 (`http://127.0.0.1:xxxx`)
  - [ ] prod 环境配置 (`https://guajunyan.top`)
  - [ ] staging 环境配置 (待定)
  - [ ] 导出当前环境判断函数

#### 1.2 状态管理
- [ ] **选择状态管理方案** (推荐 Zustand，轻量级)
  - [ ] 安装依赖: `pnpm add zustand`
  
- [ ] **创建 `src/store/userStore.ts`**
  - [ ] 用户信息状态 (token, phone, nickname)
  - [ ] 登录/登出方法
  - [ ] 持久化到 Storage
  
- [ ] **创建 `src/store/shopStore.ts`**
  - [ ] 商家信息 (营业状态、配送范围、联系方式)
  - [ ] 菜单数据缓存
  - [ ] 优惠券列表
...
#### 1.3 UI 组件库集成
- [ ] **安装 Taroify**
  - [ ] `pnpm add @taroify/core @taroify/icons`
  - [ ] 配置按需引入 (Babel 插件)
  - [ ] 主题变量配置 (CSS Vars)
  
- [ ] **创建通用组件**
  - [ ] `src/components/Skeleton/index.tsx` (骨架屏)
  - [ ] `src/components/Empty/index.tsx` (空态)
  - [ ] `src/components/ErrorBoundary/index.tsx` (错误边界)

#### 1.4 埋点框架
- [ ] **创建 `src/utils/analytics.ts`**
  - [ ] 事件上报函数 (track/page/user)
  - [ ] 数据脱敏 (手机号仅后4位)
  - [ ] 批量上报与本地缓存
