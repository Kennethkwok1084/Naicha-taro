# 🧋 奶茶点单小程序 UI/UX 规范（Taro + Taroify）

**版本**：v1.0.0-RC
**适配模式**：浅色 / 深色
**组件库**：Taroify 2.x（可替换为自定义 Icon/皮肤）
...
* 首选 **Taroify Icon**（线性风），后续可替换为自定义图标集（保持名称兼容）。
...
* **Toast**：成功 1.5s、失败 2.0s；
* **Dialog**：用于“超出配送范围 / 库存不足 / 价格变化二次确认”
* **骨架屏**：列表与规格弹层均展示骨架（图片方块 + 两行文）
* **震动**：**关闭**（不启用轻微震动）
...
> 组件映射：`NavBar`（自定义）、`Search`、`Sidebar`、`Card`、`Image`、`Tag`、`Button`、`Badge`、`Sticky`/自定义吸底。

---

### 3.2 规格弹层（由 `+` 触发 Popup）

* 容器：底部 **Popup**，高度约 70–80% 屏幕；顶部展示商品名 + 价格区域渐进更新
* 规格顺序：**（杯型参与计价但默认隐藏 UI） → 甜度 → 冰块（热饮隐藏） → 加料 → 数量**
* 交互：

  * 选项使用 `Radio`/`Tag` 可选样式（单选）
  * 加料：**最多 2 种，各 1 份**（`Checkbox` 限制 + 互斥校验文案）
  * 右侧附 `+¥x` 单价增量
  * 底部显示 **实时合计**（基础价 + 加料差价）
  * 关闭：点击遮罩或右上角 `X`；**已选状态保留**
* 操作区：`加入购物车`（次按钮）/ `立即购买`（主按钮）

> 组件映射：`Popup`、`Cell`、`Radio`、`Checkbox`、`Stepper`、`Divider`、`Button`、`Toast`。

---

### 3.3 结算页（自提 / 外送）

* 顶部 **Tabs**：`自提` / `外送`（切换触发重算）
* 自提卡片：预计时间（来自 `eta.text`，若缺失显示配置兜底文案）
* 外送卡片：

  * 地址模块：地图选点为主（手动输入兜底）；必填 **收货人 / 手机号 / 详细地址**
  * 不展示“免运费横幅”（业务设定为免运）
...
> 组件映射：`Tabs`、`Cell`、`ContactEdit`/自定义表单、`Dialog`、`Coupon` 列表（自建）、`Button`。

---

### 3.4 订单详情

* 头部：状态步进（未支付 / 制作中 / 待取 / 配送中 / 已完成）
* 核心：

  * **自提**：大号取餐码 `A102`（22/28px 等级）
  * **外送**：`预计送达 · eta.text`
* 明细：商品列表 / 金额清单 / 备注 / 地址（外送）
* 底部：`联系商家`（拨号，号源来自 `/config.contact.phone`）
...
* `NavBar`（自定义）/`Search`/`Sidebar`/`Tabs`/`Popup`/`Radio`/`Checkbox`/`Stepper`/`Button`/`Toast`/`Dialog`/`Skeleton`/`Badge`/`Image`
* 主题变量通过 CSS Vars 注入（支持 Dark/Light 切换）
